...
test('homepage contains key links', async ({ page }) => {
  ...
  const tanMockBrokerCard = page.locator('article').filter({ has: page.getByRole('heading', { name: 'TAN Mock broker login into WebMock' }) })
  ...
  await expect(page.getByLabel('TAN Mock broker login into WebMock mermaid sequence diagram')).toBeVisible()
  await expect(page.getByLabel('Refresh and logout lifecycle mermaid sequence diagram')).toBeVisible()
  await expect(page.getByLabel('TAN Mock broker login into WebMock step details')).toContainText(/prompt=none/i)
  await expect(page.getByLabel('TAN Mock broker login into WebMock step details')).toContainText(/ekw_\*/i)
...
test('tanmock admin can provision one-time tan entries and broker login into keycloak', async ({ page, browser }) => {
  ...
  await sourceIdentityCard.getByRole('button', { name: 'EKW vorbereiten' }).click()
  const tanDialog = page.getByRole('dialog', { name: `EKW fuer ${sourceUserId} anlegen` })
  ...
  if (!(await userIdField.isVisible().catch(() => false))) {
    await brokerPage.waitForURL(/tanmock\.localhost:8443\/oidc\/authorize|keycloak\.localhost:8443/, { timeout: 15000 })
  }
  ...
  await expect(brokerPage.getByLabel('Decoded access token claims')).toContainText(`ekw_${sourceUserId}_${tanHash}`)
  await expect(brokerPage.getByLabel('Decoded access token claims')).toContainText('ekw_sub')
  await expect(brokerPage.getByLabel('Decoded access token claims')).toContainText(`Ekw ${tanHash}`)
  await expect(brokerPage.getByLabel('Decoded access token claims')).toContainText('"acr": "ekw"')
