...
test('homepage contains key links', async ({ page }) => {
  ...
  await expect(tanMockBrokerCard).toContainText(/kc_idp_hint=tanmock/i)
  ...
  await expect(page.getByLabel('TAN Mock broker login into WebMock step details')).toContainText(/prompt=login/i)
  await expect(page.getByLabel('TAN Mock broker login into WebMock step details')).toContainText(/tan_\*/i)
...
test('tanmock admin can provision one-time tan entries and broker login into keycloak', async ({ page, browser }) => {
  ...
  const tanDialog = page.getByRole('dialog', { name: `TAN fuer ${sourceUserId} anlegen` })
  await tanDialog.getByRole('textbox', { name: 'TAN', exact: true }).fill(tan)
  ...
  await tanDialog.getByRole('button', { name: 'TAN speichern' }).click()
