...
const KEYCLOAK_METADATA_URL =
  "https://keycloak.localhost:8443/realms/auth-sandbox-2/.well-known/openid-configuration";
const KEYCLOAK_TOKEN_URL =
  "https://keycloak.localhost:8443/realms/auth-sandbox-2/protocol/openid-connect/token";
...
async function deleteRegistrationIdentityIfPresent(
  request: APIRequestContext,
  userId: string,
) {
  const response = await request.delete(
    `${AUTH_API_URL}/api/admin/registration-identities/${encodeURIComponent(userId)}`,
    {
...
async function cleanupTrackedTestIdentities(request: APIRequestContext) {
  ...
  await Promise.all(
    trackedIds.map((userId) =>
      deleteRegistrationIdentityIfPresent(request, userId),
    ),
  );
}
...
async function cleanupExistingTestIdentities(request: APIRequestContext) {
  ...
  const testUserIds = identities
    ...
    .filter((userId) =>
      CLEANUP_USER_PREFIXES.some((prefix) => userId.startsWith(prefix)),
    );
...
async function waitForUserLoginTrace(
  request: APIRequestContext,
  userId: string,
) {
...
async function waitForTrace(
  ...
  for (let attempt = 0; attempt < 30; attempt += 1) {
    ...
    const response = await request.get(
      `${TRACE_API_URL}/traces?${params.toString()}`,
      {
    ...
    if (response.ok()) {
      ...
      const match = body.items.find(
        (trace) =>
          trace.traceType === traceType && trace.actors.includes(actor),
      );
      if (match) {
        return match;
      }
...
async function loginWebMockWeb(
  ...
  await expect(
    page.getByRole("button", { name: /mit keycloak 1se anmelden/i }),
  ).toBeVisible();
  await page
    .getByRole("button", { name: /mit keycloak 1se anmelden/i })
    .click();
  ...
  const tokenSessionCard = page.locator(".card").filter({
    has: page.getByRole("heading", {
      name: /token claims and browser session/i,
    }),
  });
...
async function loginTanMockAdmin(page: import("@playwright/test").Page) {
  ...
  await expect(
    page.getByRole("heading", {
      name: "Registrierungsidentitaeten",
      exact: true,
    }),
  ).toBeVisible();
}
...
async function loginWithDevice(
  request: APIRequestContext,
  input: { publicKeyHash: string; privateKey: string },
) {
  const startLoginResponse = await request.post(
    `${AUTH_API_URL}/api/device/login/start`,
    {
      data: {
  ...
  const finishLoginResponse = await request.post(
    `${AUTH_API_URL}/api/device/login/finish`,
    {
      data: {
  ...
  return finishLoginResponse.json() as Promise<{
    accessToken: string;
    refreshToken: string;
  }>;
}
...
test("shared postgres runtime exposes auth, trace, and keycloak endpoints", async ({
  request,
}) => {
  const [
    healthResponse,
...
test("admin overview is localized in German", async ({ page }) => {
  ...
  await expect(
    page.getByRole("heading", {
      name: /bereite registrierungsidentitaeten vor und behalte bestehende geraetebindungen im blick/i,
    }),
  ).toBeVisible();
  await expect(
    page.getByRole("heading", {
      name: /springe vom admin-ueberblick direkt in die trace-analyse/i,
    }),
  ).toBeVisible();
  await expect(
    page.getByRole("heading", { name: "Registrierungsidentität vorbereiten" }),
  ).toBeVisible();
  await expect(
    page.getByRole("heading", { name: "Geraete", exact: true }),
  ).toBeVisible();
  await expect(page.getByLabel("Admin Ueberblickszahlen")).toContainText(
    "Geraete",
  );
  await expect(page.getByLabel("Geraete durchsuchen")).toBeVisible();
  const traceViewerLink = page.getByRole("link", {
    name: "Trace Viewer oeffnen",
  });
  await expect(traceViewerLink).toBeVisible();
  await expect(traceViewerLink).toHaveAttribute(
    "href",
...
test("admin overview shows save errors instead of opaque network failures", async ({
  page,
}) => {
  await page.goto(ADMIN_WEB_URL);
...
test("homepage contains key links", async ({ page }) => {
  await page.goto("/");
  await expect(
    page.getByRole("heading", { name: /minimal device-login sandbox/i }),
  ).toBeVisible();
  ...
  await expect(
    page.getByRole("heading", { name: /db viewer login/i }),
  ).toBeVisible();
...
test("webmock web homepage serves the browser step-up app shell", async ({
  ...
  await expect(
    page.getByRole("heading", { name: /browser login starts with 1se/i }),
  ).toBeVisible();
  await expect(
    page.getByRole("button", { name: /mit keycloak 1se anmelden/i }),
  ).toBeVisible();
  await expect(
    page.getByRole("button", { name: /step-up auf 2se starten/i }),
  ).toBeVisible();
  await expect(
    page.getByText(/trigger a fresh auth request with/i),
  ).toBeVisible();
});

test("webmock web browser login, step-up, and tracing work end to end", async ({
  page,
  request,
}) => {
  test.setTimeout(90000);
  const statusCards = page.getByLabel("WebMock status cards");
  const currentAcrCard = statusCards
    .locator("article")
    .filter({ hasText: "Current acr" });
  ...
  await expect(
    page.getByText(/step-up to 2se to unlock this endpoint/i),
  ).toBeVisible();

  await page.getByRole("button", { name: /step-up auf 2se starten/i }).click();
  await expect(page).toHaveURL(/keycloak\.localhost:8443/);
  await expect(
    page.getByText(/sms-tan bestaetigen|sms-tan bestätigen/i),
  ).toBeVisible();
  ...
  await page
    .getByRole("button", {
      name: /2se schritt abschliessen|2se Schritt abschließen/i,
    })
    .click();

  await expect(page).toHaveURL(/webmock\.localhost:8443/);
  await expect(currentAcrCard.locator("strong")).toHaveText("2se", {
    timeout: 15000,
  });
  await expect(
    page.getByText(/the token satisfied the stronger 2se assurance check/i),
  ).toBeVisible();

  await page
    .getByLabel("Neue geschützte Notiz")
    .fill("WebMock Web playwright note");
  await page
    .getByRole("button", { name: /notiz an servicemock api senden/i })
    .click();
  await expect(page.getByLabel("WebMock message response")).toContainText(
    "WebMock Web playwright note",
  );

  const internalStepUpTrace = await waitForTrace(
    request,
    userId,
    "browser_step_up_complete_internal",
    "keycloak-extension",
  );
  ...
  await timeline
    .getByRole("button", { name: /webmock_web_step_up_challenge_ready/i })
    .click();
  await expect(
    page.getByRole("tab", { name: /sms_tan_challenge event_payload/i }),
  ).toBeVisible();
  await page
    .getByRole("tab", { name: /sms_tan_challenge event_payload/i })
    .click();
  const artifactViewer = page.getByLabel("Artifact viewer");
  await expect(artifactViewer).toContainText("keycloak_inline");

  const authApiTimeline = page.getByRole("list", {
    name: "Trace spans timeline",
  });
  await expect(authApiTimeline).toContainText("auth-api");
  await expect(authApiTimeline).toContainText("keycloak-extension");
  await authApiTimeline
    .getByRole("button", {
      name: /POST \/api\/internal\/browser-step-up\/start/i,
    })
    .click();
  await expect(
    page.getByRole("tab", { name: /outgoing_response_body response_body/i }),
  ).toBeVisible();
  await page
    .getByRole("tab", { name: /outgoing_response_body response_body/i })
    .click();
  await expect(artifactViewer).toContainText("maskedTarget");
});

test("tanmock admin can provision one-time tan entries and broker login into keycloak", async ({
  page,
  browser,
}) => {
  test.setTimeout(90000);
  ...
  const identityForm = page
    ...
    .filter({
      has: page.getByRole("heading", {
        name: "Registrierungsidentität vorbereiten",
      }),
    })
    .locator("form");
  ...
  await identityForm
    .getByRole("button", { name: "Identität speichern" })
    .click();
  ...
  const identitySearch = page.getByRole("textbox", {
    name: "Identitaeten durchsuchen",
  });
  await identitySearch.fill(sourceUserId);
  const sourceIdentityCard = page
    .locator("article.admin-identity-record")
    .filter({ has: page.getByText(sourceUserId, { exact: true }) })
    .first();
  await page.waitForTimeout(650);
  await sourceIdentityCard
    .getByRole("button", { name: "EKW vorbereiten" })
    .click();
  const tanDialog = page.getByRole("dialog", {
    name: `EKW fuer ${sourceUserId} anlegen`,
  });
  await tanDialog.getByRole("textbox", { name: "EKW", exact: true }).fill(tan);
  const saveTanResponsePromise = page.waitForResponse(
    (response) =>
      response.url().includes("/tanmock-api/api/admin/entries") &&
      response.request().method() === "POST",
  );
  await tanDialog.getByRole("button", { name: "EKW speichern" }).click();
  const saveTanResponse = await saveTanResponsePromise;
  expect(saveTanResponse.ok()).toBeTruthy();
  await page.reload();
  await identitySearch.fill(sourceUserId);
  const refreshedIdentityCard = page
    .locator("article.admin-identity-record")
    .filter({ has: page.getByText(sourceUserId, { exact: true }) })
    .first();
  await expect(
    refreshedIdentityCard.getByText(tan, { exact: true }),
  ).toBeVisible();
  await expect(
    refreshedIdentityCard.getByText(tan, { exact: true }),
  ).toBeVisible();
  ...
  await brokerPage
    .getByRole("checkbox", {
      name: /TAN Mock Identity Broker statt direktem Keycloak-Login nutzen/i,
    })
    .check();
  await brokerPage
    .getByRole("button", { name: /Mit Keycloak 1se anmelden/i })
  ...
  await expect(brokerPage).toHaveURL(
    /keycloak\.localhost:8443|tanmock\.localhost:8443/,
    { timeout: 15000 },
  );
  const userIdField = brokerPage.getByLabel("User ID");
  if (!(await userIdField.isVisible().catch(() => false))) {
    await brokerPage.waitForURL(
      /tanmock\.localhost:8443\/oidc\/authorize|keycloak\.localhost:8443/,
      { timeout: 15000 },
    );
  }
  await userIdField.fill(sourceUserId);
  await brokerPage.getByLabel("EKW").fill(tan);
  await brokerPage
    .getByRole("button", { name: "Anmeldung fortsetzen" })
    .click();

  await expect(brokerPage).toHaveURL(/webmock\.localhost:8443/);
  await expect(
    brokerPage.locator(".card").filter({
      has: brokerPage.getByRole("heading", {
        name: /token claims and browser session/i,
      }),
    }),
  ).toContainText("yes", { timeout: 15000 });
  await expect(
    brokerPage.getByLabel("Decoded access token claims"),
  ).toContainText(`ekw_${sourceUserId}_${tanHash}`);
  await expect(
    brokerPage.getByLabel("Decoded access token claims"),
  ).toContainText("ekw_sub");
  await expect(
    brokerPage.getByLabel("Decoded access token claims"),
  ).toContainText(`Ekw ${tanHash}`);
  await expect(
    brokerPage.getByLabel("Decoded access token claims"),
  ).toContainText('"acr": "ekw"');

  await brokerContext.close();
});

test("appmock can open webmock through bootstrap SSO", async ({
  page,
  context,
  request,
}) => {
  ...
  const startSmsTanResponsePromise = page.waitForResponse(
    (response) =>
      response.url().includes("/api/identification/sms-tan/start") &&
      response.request().method() === "POST",
  );
  await page
    .getByRole("button", { name: "SMS-TAN senden", exact: true })
    .click();
  const startSmsTanResponse = await startSmsTanResponsePromise;
  const startSmsTanBody =
    (await startSmsTanResponse.json()) as SmsTanStartResponse;
  await page
    .getByRole("textbox", { name: "SMS-TAN" })
    .fill(startSmsTanBody.devCode ?? "");
  ...
  const prepareWebmockSsoButton = page
    .locator("button")
    .filter({ hasText: "WebMock SSO vorbereiten" })
    .first();
  await expect(prepareWebmockSsoButton).toBeVisible({ timeout: 20000 });

  const securePrompt = page.getByRole("region", {
    name: "Secure element prompt",
  });
  ...
  await expect(
    page.getByRole("note", { name: "SSO launch result" }),
  ).toHaveCount(0);
  ...
  await expect(ssoLaunchNote).toContainText(
    "SSO-Start: https://keycloak.localhost:8443/",
  );
  await expect(ssoLaunchNote).toContainText("https://webmock.localhost:8443/");
  await expect(
    page.getByRole("button", { name: "SSO-URL kopieren" }),
  ).toBeVisible();
  await page
    .getByRole("button", { name: "Vorbereiteten SSO-Tab öffnen" })
    .click();
  const webmockPage = await newPagePromise;
  await webmockPage.waitForLoadState("networkidle");

  await expect(webmockPage).toHaveURL(
    /webmock\.localhost:8443|auth\.localhost:8443\/api\/sso-bootstrap\/callback|keycloak\.localhost:8443/,
  );

  const currentUrl = webmockPage.url();
  expect(currentUrl).not.toContain(
    "/protocol/openid-connect/auth?client_id=sso-bootstrap-web&request_uri=",
  );

  if (currentUrl.includes("webmock.localhost:8443")) {
    const webmockStatusCards = webmockPage.getByLabel("WebMock status cards");
    const currentAcrCard = webmockStatusCards
      .locator("article")
      .filter({ hasText: "Current acr" });
    const tokenSessionCard = webmockPage.locator(".card").filter({
      has: webmockPage.getByRole("heading", {
        name: /token claims and browser session/i,
      }),
    });
    ...
    await expect(
      webmockPage
        .getByRole("heading", {
          name: /Browser login starts with 1se|Token claims and browser session/i,
        })
        .first(),
    ).toBeVisible();
    await expect(currentAcrCard.locator("strong")).toHaveText("none", {
      timeout: 15000,
    });
    await expect(
      webmockPage.getByRole("button", { name: /mit keycloak 1se anmelden/i }),
    ).toBeVisible();
    ...
    await webmockPage.waitForLoadState("networkidle");
    await expect(webmockPage).toHaveURL(
      /keycloak\.localhost:8443|webmock\.localhost:8443/,
    );
    expect(webmockPage.url()).not.toContain("request_uri=");
    await expect(webmockPage.getByText("Sign in to your account")).toHaveCount(
      0,
    );
    await expect(currentAcrCard.locator("strong")).toHaveText("1se", {
      timeout: 15000,
    ...
    await expect(
      webmockPage.getByText(/step-up to 2se to unlock this endpoint/i),
    ).toBeVisible();
    await webmockPage
      .getByRole("button", { name: /step-up auf 2se starten/i })
      .click();
    await webmockPage.waitForLoadState("networkidle");
    await expect(webmockPage.getByText("Sign in to your account")).toHaveCount(
      0,
  ...
  const ssoTrace = await waitForTrace(
    request,
    userId,
    "sso_launch_finished",
    "appmock-web",
  ...
  await ssoTimeline
    .getByRole("button", {
      name: /keycloak POST \/realms\/auth-sandbox-2\/protocol\/openid-connect\/ext\/par\/request/i,
    })
    .click();
  await expect(
    page.getByRole("tab", { name: /request_body request_body/i }),
  ).toBeVisible();
  await page.getByRole("tab", { name: /request_body request_body/i }).click();
  await expect(page.getByLabel("Artifact viewer")).toContainText(
    "sso-bootstrap-web",
  );
});

test("device login flow supports tokens refresh and logout", async ({
  page,
  request,
}) => {
  ...
  await page.goto("https://appmock.localhost:8443");
  await expect(
    page.getByRole("heading", { name: "Dieses Telefon einrichten" }),
  ).toBeVisible();
  await expect(
    page.getByRole("heading", { name: "Geräteanmeldung einrichten" }),
  ).toBeVisible();
  await expect(
    page.getByRole("heading", { name: "Sitzungstokens", exact: true }),
  ).toBeVisible();
  await expect(page.getByLabel("Token wallet empty state")).toContainText(
    "Tokens erscheinen erst nach erfolgreicher Geräteanmeldung.",
  );
  await page.getByLabel("Benutzer-ID").fill(userId);
  ...
  await expect(page.getByLabel("Nächster Schritt")).toHaveValue(
  ...
  await expect(
    page.getByText(`SMS-TAN fuer die Demo: ${startSmsTanBody.devCode ?? ""}`),
  ).toBeVisible();
  await expect(
    page.getByRole("button", { name: "Neue SMS-TAN senden" }),
  ).toBeVisible();
  await page
    .getByRole("textbox", { name: "SMS-TAN" })
    .fill(startSmsTanBody.devCode ?? "");
  ...
  await expect(
    page.getByText(
      "Gerätebindung gespeichert. Lege ein neues Keycloak-Passwort fest, um fortzufahren.",
    ),
  ).toBeVisible();
  await expect(
    page.getByRole("heading", { name: "Neues Passwort erstellen" }),
  ).toBeVisible();
  await expect(
    page.getByRole("button", { name: "Passwort speichern" }),
  ).toBeVisible();
  ...
  await expect(
    page
      .getByLabel("Token claim summary")
      .locator("article")
      .filter({ hasText: "Assurance Level" })
      .locator("strong"),
  ).toHaveText("2se");
  ...
  await page.reload();
  await expect(
    page.getByRole("heading", { name: "Gerät bereit" }),
  ).toBeVisible();
  await expect(
  ...
  await expect(bindingNotice).toContainText(
    "private Schlüssel bleibt auf diesem Gerät",
  );
  await expect(
    page.getByRole("button", { name: "Mit Gerät fortfahren" }),
  ).toBeVisible();
  await expect(
    page.getByRole("button", { name: "Gerätebindung entfernen" }),
  ).toBeVisible();
  ...
  await expect(
    page.getByRole("tab", { name: "ServiceMock API Demo API" }),
  ).toHaveAttribute("aria-selected", "false");
  await expect(page.getByLabel("Token claim summary")).toBeVisible();
  await expect(
    page.getByRole("heading", { name: "Userinfo-Endpunkt" }),
  ).toBeVisible();
  await expect(
    page.getByRole("heading", { name: "Introspection-Endpunkt" }),
  ).toBeVisible();
  await expect(
    page.getByRole("heading", { name: "Refresh-Token" }),
  ).toBeVisible();
  await expect(page.getByLabel("Authenticated token summary")).toContainText(
    "Token-Typ",
  );
  const userInfoPanel = page
    .locator(".token-panel")
    .filter({ has: page.getByRole("heading", { name: "Userinfo-Endpunkt" }) });
  const introspectionPanel = page.locator(".token-panel").filter({
    has: page.getByRole("heading", { name: "Introspection-Endpunkt" }),
  });
  const userInfoSummary = page.getByLabel("Userinfo-Endpunkt summary");
  const introspectionSummary = page.getByLabel(
    "Introspection-Endpunkt summary",
  );
  ...
  const comparisonClaimsTable = page.getByRole("table", {
    name: "Access- und ID-Token Claims",
  });
  await expect(claimSummary).toBeVisible();
  await expect(comparisonClaimsTable).toBeVisible();
  await expect(
    comparisonClaimsTable.getByRole("row", { name: /exp/i }),
  ).toContainText("Unix");
  await expect(
    page.locator("summary").filter({ hasText: /Token JWT/ }),
  ).toHaveCount(3);
  await expect(
    comparisonClaimsTable.getByRole("row", { name: /email_verified/i }),
  ).toContainText("false");
  await expect(
    claimSummary
      .locator("article")
      .filter({ hasText: "Benutzer-ID" })
      .locator("strong"),
  ).toHaveText(userId);
  await expect(
    claimSummary
  ...
  await expect(
    claimSummary
      .locator("article")
      .filter({ hasText: "Sitzungs-ID" })
      .locator("strong"),
  ).not.toBeEmpty();
  await expect(
    claimSummary
      .locator("article")
      .filter({ hasText: "Rollen" })
      .locator("strong"),
  ).not.toBeEmpty();
  await expect(
    claimSummary
  ...
  await expect(
    claimSummary
      .locator("article")
      .filter({ hasText: "Endet" })
      .locator("strong"),
  ).not.toBeEmpty();
  await expect(
    comparisonClaimsTable.getByRole("row", { name: /acr/i }),
  ).toContainText("2se");
  ...
  await expect(
    page.getByRole("button", { name: "ServiceMock API neu laden" }),
  ).toBeVisible();
  await expect(page.getByText("OIDC-geschützte Demo-Endpunkte")).toBeVisible();
  await page
    .getByLabel("Neue geschützte Notiz")
    .fill("Playwright protected note");
  await page
    .getByRole("button", { name: "Notiz an ServiceMock API senden" })
    .click();
  await expect(serviceMockApiPanel).toContainText("Playwright protected note");
  await waitForTrace(
    request,
    userId,
    "servicemock_api_message_create_finished",
    "servicemock-api",
  );
  ...
  await expect(
    page.getByRole("note", { name: "Local device binding notice" }),
  ).toContainText(
    "Tokens werden aber erst nach einer erfolgreichen Geräteanmeldung ausgestellt.",
  );

  await page.getByRole("button", { name: "Gerätebindung entfernen" }).click();
  await expect(
    page.getByText("Gerätebindung von diesem Gerät entfernt"),
  ).toBeVisible();
  ...
  await expect(
    page.getByRole("heading", {
      name: /behalte den ausgewaehlten trace im blick und öffne die detailinspektion/i,
    }),
  ).toBeVisible();
  await expect(
    page.getByText(/im demo-modus werden alle payloads erfasst/i),
  ).toBeVisible();

  const traceList = page.getByRole("list", { name: "Trace list" });
  await expect(traceList).toContainText(
    /device_login_finish|device_login_finished/i,
  );
  ...
  await expect(
    page.getByText(/Diese Seite zeigt Requests und Responses je Span/i),
  ).toBeVisible();
  await expect(
    page.locator('[aria-label="Trace Zusatzmetadaten"]').first(),
  ).toContainText(loginTrace.traceId);
  await expect(
    page.locator('[aria-label="Trace Zusatzmetadaten"]').first(),
  ...
  await expect(
    timeline.getByText(/\d{2}\.\d{2}\.\d{4}.*UTC/i).first(),
  ).toBeVisible();

  await timeline.getByRole("button", { name: /auth-api start_login/i }).click();
  const artifactTabs = page.getByRole("tablist", {
    name: "Artifact quick access",
  });
...
test("registration and step-up flow APIs support service selection, concrete service lifecycle, finalize, and redeem", async ({
  page,
  request,
}) => {
  ...
  const registrationIdentityResponse = await request.post(
    `${AUTH_API_URL}/api/admin/registration-identities`,
    {
      headers: ADMIN_PROXY_HEADERS,
      data: {
  ...
  const createRegistrationFlow = await request.post(
  ...
  const missingTokenGet = await request.get(
    `${AUTH_API_URL}/api/flows/${registrationFlow.flowId}`,
  );
  expect(missingTokenGet.status()).toBe(401);

  const selectCode = await request.post(
    `${AUTH_API_URL}/api/flows/${registrationFlow.flowId}/select-service`,
    {
      headers: withBearer(registrationFlow.flowToken),
      data: {
        service: "person_code",
  ...
  const selectedCode = (await selectCode.json()) as {
    serviceToken: string;
  ...
  const completeCode = await request.post(
  ...
  const completedCode = (await completeCode.json()) as {
    status: string;
  ...
  const finalizeRegistration = await request.post(
    `${AUTH_API_URL}/api/flows/${registrationFlow.flowId}/finalize`,
    {
      headers: withBearer(registrationFlow.flowToken),
      data: {
  ...
  const finalizedRegistration = (await finalizeRegistration.json()) as {
    finalization: {
      kind: string;
      userId: string;
  ...
  expect(finalizedRegistration.finalization.publicKeyHash).toBe(
    signingKeys.publicKeyHash,
  );
  ...
  const anonymousStepUpFlow = await request.post(
    `${AUTH_API_URL}/api/step-up-flows`,
    {
      data: {
        userId,
        requiredAcr: "level_1",
  ...
  const createStepUpFlow = await request.post(
    `${AUTH_API_URL}/api/step-up-flows`,
    {
  ...
  const stepUpFlow = (await createStepUpFlow.json()) as {
    flowId: string;
    flowToken: string;
    availableServices: Array<{ id: string }>;
  };
  expect(stepUpFlow.availableServices.map((service) => service.id)).toEqual([
    "sms_tan",
  ]);

  const selectSms = await request.post(
    `${AUTH_API_URL}/api/flows/${stepUpFlow.flowId}/select-service`,
    {
  ...
  const startSms = await request.post(
    `${AUTH_API_URL}/api/identification/sms-tan/start`,
    {
  ...
  const startedSms = (await startSms.json()) as {
  ...
  const resendSms = await request.post(
    `${AUTH_API_URL}/api/identification/sms-tan/resend`,
    {
      headers: withBearer(selectedSms.serviceToken),
      data: {},
    },
  ...
  const completeSms = await request.post(
    `${AUTH_API_URL}/api/identification/sms-tan/complete`,
    {
      headers: withBearer(selectedSms.serviceToken),
      data: {
  ...
  const completedSms = (await completeSms.json()) as {
    serviceResultToken: string;
    achievedAcr: string;
  };
  expect(completedSms.achievedAcr).toBe("level_2");

  const finalizeBrowser = await request.post(
    `${AUTH_API_URL}/api/flows/${stepUpFlow.flowId}/finalize`,
    {
      headers: withBearer(stepUpFlow.flowToken),
      data: {
  ...
  const finalizedBrowser = (await finalizeBrowser.json()) as {
    finalization: { kind: string; resultCode: string };
  };
  expect(finalizedBrowser.finalization.kind).toBe("result_code");

  const unauthorizedRedeemResultCode = await request.post(
    `${AUTH_API_URL}/api/internal/flows/redeem`,
    {
      data: {
  ...
  const redeemResultCode = await request.post(
    `${AUTH_API_URL}/api/internal/flows/redeem`,
    {
      headers: {
  ...
  const redeemedResult = (await redeemResultCode.json()) as {
    userId: string;
  ...
  await flowTimeline
    .getByRole("button", { name: /POST \/api\/registration-flows/i })
    .click();

  const flowArtifactTabs = page.getByRole("tablist", {
    name: "Artifact quick access",
  });
  await expect(flowArtifactTabs).toContainText("outgoing_response_body");
  await flowArtifactTabs
    .getByRole("tab", { name: /^outgoing_response_body/i })
    .click();
  ...
  await expect(flowArtifactViewer).toContainText(
    "Verschachtelt decodiert: body.flowToken",
  );
...
test("missing saved device binding is cleared instead of failing with a server error", async ({
  page,
  request,
}) => {
  ...
  await page
    ...
    .fill(registrationCode);
  await page
    .getByRole("button", { name: "Identifikation abschließen" })
    .click();

  await expect(
    page.getByRole("heading", {
      name: /Neues Passwort erstellen|Mit gespeichertem Gerät anmelden/,
    }),
  ).toBeVisible();
  if (
    await page
      .getByRole("heading", { name: "Neues Passwort erstellen" })
      .isVisible()
      .catch(() => false)
  ) {
    await page.getByLabel("Neues Passwort").fill("ChangeMe123!");
  ...
  const deleteResponse = await request.delete(
    `${AUTH_API_URL}/api/admin/devices/${device?.id}`,
    {
      headers: ADMIN_PROXY_HEADERS,
    },
  );
  ...
  await expect(
    page.getByText(
      "Gespeicherte Gerätebindung war ungültig und wurde entfernt",
    ),
  ).toBeVisible();
  await expect(
    page.getByRole("heading", { name: "Dieses Telefon einrichten" }),
...
test("ekw broker login is single-use - second login with same ekw fails", async ({
  page,
  browser,
}) => {
  test.setTimeout(90000);

  const userId = trackCleanupUserId(`ekwreuse-user-${Date.now()}`);
  const sourceUserId = trackCleanupUserId(`ekwreuse-source-${Date.now()}`);
  const tan = `${Math.floor(Math.random() * 900000 + 100000)}`;
  ...
  const identityForm = page
    ...
    .locator("form");
  await identityForm.locator('input[name="userId"]').fill(sourceUserId);
  await identityForm.locator('input[name="firstName"]').fill("EKW");
  await identityForm.locator('input[name="lastName"]').fill("Reuse");
  await identityForm.locator('input[name="birthDate"]').fill("1991-02-03");
  await identityForm.locator('input[name="phoneNumber"]').fill("+491701234568");
  await identityForm
    .getByRole("button", { name: "Identität speichern" })
  ...
  // Create target user identity
  await identityForm.locator('input[name="userId"]').fill(userId);
  await identityForm.locator('input[name="firstName"]').fill("Target");
  await identityForm.locator('input[name="lastName"]').fill("User");
  await identityForm.locator('input[name="birthDate"]').fill("1990-01-01");
  await identityForm.locator('input[name="phoneNumber"]').fill("+491701234567");
  await identityForm
  ...
  // First login attempt with the EKW - should succeed
  const brokerContext1 = await browser.newContext({ ignoreHTTPSErrors: true });
  const brokerPage1 = await brokerContext1.newPage();
  await brokerPage1.goto(MOCK_WEB_URL);
  await brokerPage1
    .getByRole("checkbox", {
      name: /TAN Mock Identity Broker statt direktem Keycloak-Login nutzen/i,
    })
  ...
  await brokerPage1
    .getByRole("button", { name: /Mit Keycloak 1se anmelden/i })
    .click();
  await brokerPage1.waitForURL(
    /tanmock\.localhost:8443\/oidc\/authorize|keycloak\.localhost:8443/,
    { timeout: 15000 },
  );
  const userIdField1 = brokerPage1.getByLabel("User ID");
  await userIdField1.fill(sourceUserId);
  await brokerPage1.getByLabel("EKW").fill(tan);
  await brokerPage1
    .getByRole("button", { name: "Anmeldung fortsetzen" })
    .click();
  await expect(brokerPage1).toHaveURL(/webmock\.localhost:8443/, {
    timeout: 15000,
  });
  await expect(
    brokerPage1.locator(".card").filter({
      has: brokerPage1.getByRole("heading", {
        name: /token claims and browser session/i,
      }),
    }),
  ).toContainText("yes");

  // Second login attempt with the same EKW - should fail
  const brokerContext2 = await browser.newContext({ ignoreHTTPSErrors: true });
  const brokerPage2 = await brokerContext2.newPage();
  await brokerPage2.goto(MOCK_WEB_URL);
  await brokerPage2
    .getByRole("checkbox", {
      name: /TAN Mock Identity Broker statt direktem Keycloak-Login nutzen/i,
    })
  ...
  await brokerPage2
    .getByRole("button", { name: /Mit Keycloak 1se anmelden/i })
    .click();
  await brokerPage2.waitForURL(
    /tanmock\.localhost:8443\/oidc\/authorize|keycloak\.localhost:8443/,
    { timeout: 15000 },
  );
  const userIdField2 = brokerPage2.getByLabel("User ID");
  await userIdField2.fill(sourceUserId);
  await brokerPage2.getByLabel("EKW").fill(tan);
  await brokerPage2
    .getByRole("button", { name: "Anmeldung fortsetzen" })
    .click();
  ...
  await brokerContext2.close();
});

test("ekw sso cookie cannot be reused after first login - single-use enforcement", async ({
  page,
  browser,
}) => {
  test.setTimeout(90000);

  const userId = trackCleanupUserId(`ekwcookie-user-${Date.now()}`);
  const sourceUserId = trackCleanupUserId(`ekwcookie-source-${Date.now()}`);
  const tan = `${Math.floor(Math.random() * 900000 + 100000)}`;
  ...
  const identityForm = page
    ...
    .locator("form");
  await identityForm.locator('input[name="userId"]').fill(sourceUserId);
  await identityForm.locator('input[name="firstName"]').fill("Cookie");
  await identityForm.locator('input[name="lastName"]').fill("Test");
  await identityForm.locator('input[name="birthDate"]').fill("1991-02-03");
  await identityForm.locator('input[name="phoneNumber"]').fill("+491701234568");
  await identityForm
    .getByRole("button", { name: "Identität speichern" })
  ...
  // Create EKW for source user with webmock-web as allowed target
  const identitySearch = page.getByRole("textbox", {
    name: "Identitaeten durchsuchen",
  });
  await identitySearch.fill(sourceUserId);
  ...
  // First EKW login - should succeed and establish SSO cookie
  const brokerContext1 = await browser.newContext({
  ...
  // Cookie should be set after first login - capture the storage state
  const cookieState = await brokerContext1.storageState();

  // Second login attempt in same context (cookie reuse) - should fail with prompt=none
  // Navigate to EKW login page again - browser-step-up-flow should detect the consumed EKW cookie
  await brokerPage1.goto(MOCK_WEB_URL + "/#/login");
  await brokerPage1.waitForLoadState("networkidle");

  // The EKW session should be marked as consumed, so the cookie-based SSO should be rejected
  // Either an error page or re-login should be required
  const currentUrl = brokerPage1.url();
  // If the cookie is still valid but consumed, the step-up flow should reject with ACCESS_DENIED
  // or the session should require re-authentication
