...
import type {
  ServiceMockApiAssuranceLevel,
  ServiceMockApiAssuranceResponse,
  ServiceMockApiCreateMessageInput,
  ServiceMockApiCreateMessageResponse,
  ServiceMockApiMessageRecord,
  ServiceMockApiMessagesResponse,
...
const jwks = createRemoteJWKSet(new URL(serviceMockApiConfig.jwksUrl))
const createMessageSchema = z.object({
  text: z.string().trim().min(1).max(280)
})

const messagesByUser = new Map<string, ServiceMockApiMessageRecord[]>()
...
async function verifyAccessToken(request: FastifyRequest) {
  ...
  const { payload } = await jwtVerify(token, jwks, {
    issuer: serviceMockApiConfig.issuer,
    audience: serviceMockApiConfig.audience
  })
...
function satisfiesRequiredLevel(acr: string | null, requiredLevel: ServiceMockApiAssuranceLevel) {
...
async function requireAssuranceLevel(request: FastifyRequest, requiredLevel: ServiceMockApiAssuranceLevel) {
...
function buildAssuranceResponse(claims: MockAccessTokenClaims, requiredLevel: ServiceMockApiAssuranceLevel): ServiceMockApiAssuranceResponse {
  ...
  return {
    ...
    issuer: typeof claims.iss === 'string' ? claims.iss : serviceMockApiConfig.issuer,
...
function getMessages(userId: string) {
  ...
  const seeded: ServiceMockApiMessageRecord[] = [
    {
      id: `seed-${userId}`,
...
function createMessage(userId: string, input: ServiceMockApiCreateMessageInput) {
  const messages = getMessages(userId)
  const item: ServiceMockApiMessageRecord = {
    id: randomUUID(),
    text: input.text,
    authorUserId: userId,
    createdAt: new Date().toISOString(),
...
export async function registerMockRoutes(app: any) {
  app.get('/health', async () => ({ status: 'ok', service: 'servicemock-api' }))
  ...
  app.get('/api/mock/assurance/1se', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    return tracedRoute({
      request,
      reply,
      traceType: 'servicemock_api_assurance_1se_read',
      title: `Mock 1se assurance check for ${userId}`,
      summary: 'servicemock-api accepted the token for a baseline assurance-protected endpoint.',
      userId,
  ...
  app.get('/api/mock/assurance/2se', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    return tracedRoute({
      request,
      reply,
      traceType: 'servicemock_api_assurance_2se_read',
      title: `Mock 2se assurance check for ${userId}`,
      summary: 'servicemock-api accepted the token for a strong assurance-protected endpoint.',
      userId,
  ...
  app.get('/api/mock/profile', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
      ...
      reply.code(401)
      return { message: 'OIDC token missing, invalid, or audience does not match servicemock-api' }
    }
    ...
    return tracedRoute({
      request,
      reply,
      traceType: 'servicemock_api_profile_read',
      title: `Mock profile for ${userId}`,
      summary: 'servicemock-api accepted the Keycloak access token after JWKS signature and audience validation.',
      userId,
      run: async (): Promise<ServiceMockApiProfileResponse> => ({
        ...readTraceEnvelope(),
        subject: claims.sub ?? userId,
        userId,
        username: claims.preferred_username ?? userId,
  ...
  app.get('/api/mock/messages', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    return tracedRoute({
      request,
      reply,
      traceType: 'servicemock_api_messages_list',
      title: `Mock messages for ${userId}`,
      summary: 'appmock-web loaded protected servicemock-api data with the current access token.',
      userId,
      run: async (): Promise<ServiceMockApiMessagesResponse> => ({
        ...readTraceEnvelope(),
  ...
  app.post('/api/mock/messages', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    const result = await tracedRoute({
      request,
      reply,
      traceType: 'servicemock_api_message_create',
      title: `Create mock message for ${userId}`,
      summary: 'appmock-web posted a protected servicemock-api request using the current Keycloak access token.',
      userId,
      body,
      run: async (): Promise<ServiceMockApiCreateMessageResponse> => ({
        ...readTraceEnvelope(),
