...
const SERVICEMOCK_API_BASE = import.meta.env.VITE_SERVICEMOCK_API_URL ?? '/servicemock-api'
...
async function mockRequest<T>(path: string, accessToken: string, init?: RequestInit, options?: TraceRequestOptions) {
  return requestJson<T>(SERVICEMOCK_API_BASE, path, {
    ...init,
    headers: {
...
export const api = {
  ...
  checkDeviceConflict: (body: { userId: string; deviceName: string }, options?: TraceRequestOptions) => requestJson<DeviceConflictCheckResponse>(API_BASE, '/api/device/conflicts/check', { method: 'POST', body: JSON.stringify(body) }, options),
  deleteDeviceConflict: (id: string, body: { userId: string; conflictToken: string }, options?: TraceRequestOptions) => requestJson<void>(API_BASE, `/api/device/conflicts/${id}`, { method: 'DELETE', body: JSON.stringify(body) }, options),
  ...
  completeMobileStepUp: (body: CompleteMobileStepUpInput, accessToken: string, options?: TraceRequestOptions) => bearerRequest<{ flow: PublicAssuranceFlowRecord; tokens: RefreshTokensResponse }>('/api/step-up/mobile/complete', accessToken, { method: 'POST', body: JSON.stringify(body) }, options),
  ...
  getMockProfile: (accessToken: string, options?: TraceRequestOptions) => mockRequest<ServiceMockApiProfileResponse>('/api/mock/profile', accessToken, undefined, options),
  listMockMessages: (accessToken: string, options?: TraceRequestOptions) => mockRequest<ServiceMockApiMessagesResponse>('/api/mock/messages', accessToken, undefined, options),
  createMockMessage: (accessToken: string, body: ServiceMockApiCreateMessageInput, options?: TraceRequestOptions) => mockRequest<ServiceMockApiCreateMessageResponse>('/api/mock/messages', accessToken, { method: 'POST', body: JSON.stringify(body) }, options)
}
