...
import { buildAuthorizationUrl, getMockApiAccessLabel, satisfiesAssuranceLevel } from './state'

describe('mock web helpers', () => {
  it('builds a browser authorization request with acr values', () => {
    const url = buildAuthorizationUrl({
      ...
      redirectUri: 'https://mock.localhost:8443/',
      acrValues: '2se',
      state: 'state-1',
      nonce: 'nonce-1'
    })
    ...
    expect(url).toContain('client_id=browser-app')
  })
  ...
  it('describes API access based on current acr', () => {
    expect(getMockApiAccessLabel('2se')).toContain('1se als auch 2se')
    expect(getMockApiAccessLabel('1se')).toContain('nur 1se')
  })
})
