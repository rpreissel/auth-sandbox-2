...
import type { MockApiAssuranceResponse, MockApiMessagesResponse, MockApiProfileResponse } from '@auth-sandbox-2/shared-types'

import './styles.css'
import { buildAuthorizationUrl, getMockApiAccessLabel, normalizeAcr, normalizeAmr } from './state'

const KEYCLOAK_BASE = 'https://keycloak.localhost:8443/realms/auth-sandbox-2/protocol/openid-connect'
const MOCK_API_BASE = import.meta.env.VITE_MOCK_API_URL ?? '/mock-api/api/mock'
const CLIENT_ID = import.meta.env.VITE_KEYCLOAK_CLIENT_ID ?? 'browser-app'
const REDIRECT_URI = 'https://mock.localhost:8443/'
const TOKEN_STORAGE_KEY = 'auth-sandbox-2.mock-web.tokens'
  ...
  await fetch(`${TRACE_API_BASE}/client-events`, {
    ...
    body: JSON.stringify({
      ...
      artifacts: input.artifacts?.map((artifact) => ({
        ...
        explanation: artifact.explanation ?? 'Client-side artifact captured by mock-web.'
      }))
    })
  })
}
...
function MockWebApp() {
  ...
  const [profile, setProfile] = useState<MockApiProfileResponse | null>(null)
  const [assurance1, setAssurance1] = useState<MockApiAssuranceResponse | null>(null)
  const [assurance2, setAssurance2] = useState<MockApiAssuranceResponse | null>(null)
  const [messages, setMessages] = useState<MockApiMessagesResponse | null>(null)
  ...
  async function loadProtectedData() {
    ...
    try {
      await sendClientEvent({
        trace,
        operation: 'mock_web_mock_api_load_started',
        traceType: 'mock_web_mock_api_load',
      ...
      const [profileResult, messagesResult, assurance1Result] = await Promise.all([
        requestJson<MockApiProfileResponse>(`${MOCK_API_BASE}/profile`, tokens.accessToken, trace),
        requestJson<MockApiMessagesResponse>(`${MOCK_API_BASE}/messages`, tokens.accessToken, trace),
        requestJson<MockApiAssuranceResponse>(`${MOCK_API_BASE}/assurance/1se`, tokens.accessToken, trace)
      ])
      ...
      try {
        const assurance2Result = await requestJson<MockApiAssuranceResponse>(`${MOCK_API_BASE}/assurance/2se`, tokens.accessToken, trace)
      ...
      await sendClientEvent({
        trace,
        operation: 'mock_web_mock_api_load_finished',
        traceType: 'mock_web_mock_api_load',
      ...
      setMessageState({ kind: 'info', text: 'Mock API data refreshed.' })
    } catch (error) {
      setMessageState({ kind: 'error', text: error instanceof Error ? error.message : 'Mock API request failed.' })
    } finally {
      setLoading(false)
    }
  }
  ...
  async function createNote() {
    ...
    try {
      await sendClientEvent({
        trace,
        operation: 'mock_web_note_create_started',
        traceType: 'mock_web_note_create',
        userId: typeof tokenClaims?.preferred_username === 'string' ? tokenClaims.preferred_username : null,
        artifacts: [
          {
            artifactType: 'event_payload',
      ...
      await fetch(`${MOCK_API_BASE}/messages`, {
        method: 'POST',
        headers: {
      ...
      await sendClientEvent({
        trace,
        operation: 'mock_web_note_create_finished',
        traceType: 'mock_web_note_create',
        userId: typeof tokenClaims?.preferred_username === 'string' ? tokenClaims.preferred_username : null
      })
      setNewNote('')
  ...
  async function startInteractiveStepUp() {
    ...
    try {
      await sendClientEvent({
        trace,
        operation: 'mock_web_step_up_started',
        traceType: 'mock_web_step_up',
        userId: typeof tokenClaims?.preferred_username === 'string' ? tokenClaims.preferred_username : null,
        artifacts: [
          {
            artifactType: 'event_payload',
      ...
      await sendClientEvent({
        trace,
        operation: 'mock_web_step_up_challenge_ready',
        traceType: 'mock_web_step_up',
        userId: typeof tokenClaims?.preferred_username === 'string' ? tokenClaims.preferred_username : null,
        artifacts: [
          {
  ...
  function logout() {
    persistTokens(null)
    setTokens(null)
    setMessageState({ kind: 'warn', text: 'Local mock-web tokens cleared. Log out in Keycloak separately if you want to remove the SSO cookie.' })
  }

  return (
    <main className="shell">
      <section className="hero stack">
        <p className="eyebrow">Mock Web</p>
        <h1>Browser login starts with 1se and can later step up to 2se through a fresh Keycloak auth request.</h1>
        <p>This demo keeps the first browser sign-in at password level and only upgrades to 2se when you explicitly trigger step-up for the stronger mock-api endpoint.</p>
        <div className="status-list" aria-label="Mock web status cards">
          ...
          <article className="status-item">
            ...
            <strong>{getMockApiAccessLabel(currentAcr)}</strong>
          </article>
        </div>
      ...
      <section className="shell-grid">
        <section className="stack">
          <section className="card stack">
            ...
            <div className="token-grid">
              ...
              <article className="token-item">
                <span>Client</span>
                <strong>{typeof tokenClaims?.azp === 'string' ? tokenClaims.azp : 'browser-app'}</strong>
              </article>
              <article className="token-item">
                <span>Expires</span>
          ...
          <section className="card stack">
            <p className="eyebrow">Protected Mock API</p>
          ...
          <section className="card stack">
            ...
            <h2>Post a demo note through the existing mock-api</h2>
            <div className="form-grid">
              ...
              <div className="button-row">
                <button type="button" onClick={() => void createNote()} disabled={!tokens || loading || !newNote.trim()}>Notiz an Mock API senden</button>
                <button type="button" className="secondary" onClick={() => void loadProtectedData()} disabled={!tokens || loading}>Mock API neu laden</button>
              </div>
            </div>
            <pre className="json-block" aria-label="Mock message response">{JSON.stringify(messages, null, 2)}</pre>
          </section>
        </section>
        ...
        <section className="stack">
          ...
          <section className="card stack">
            ...
            <div className="tan-card">
              <span>Tracing</span>
              <strong>Every mock-api request already emits traces. Browser step-up traces need to be validated once the end-to-end redirect path is finished.</strong>
            </div>
          </section>
        </section>
      </section>
