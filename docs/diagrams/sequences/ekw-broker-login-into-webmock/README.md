...
6. **Silent SSO with prompt=none** (WebMock Web → Keycloak (webmock client)): WebMock detects the ekw: state prefix in the callback and immediately starts a fresh authorization request against the webmock-web client with prompt=none and `ekw_handoff=1`. Keycloak reuses the browser session from the EKW bootstrap exactly once for the configured target client.
